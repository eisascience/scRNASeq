...
AnnotationFiles <- list.files("./10X/bin", pattern = "QuickGO", full.names = T)
GeneLists <- list()
...
for(AnnFile in AnnotationFiles){
  ...
  # if("row.names" %in% colnames(GeneLists$Extra[[tempName]])){
  #   tempColnames <- setdiff(colnames(GeneLists$Extra[[tempName]]), "row.names")
  #   GeneLists$Extra[[tempName]] <- GeneLists$Extra[[tempName]][,1:(ncol(GeneLists$Extra[[tempName]])-1)]
  #   colnames(GeneLists$Extra[[tempName]]) <- tempColnames
  # }
  
  
}
HumanExtraGOgenes <- data.table::rbindlist(lapply(GeneLists$Extra, function(setX){
  subset(setX, TAXON.ID == 9606)[,c("GO.NAME", "SYMBOL")] #10090 = mouse ; 9606 = human
}))

BiologicallyMeaningfulSet <- unique(c(gsub("_", "", gsub("-", "", as.character(HumanExtraGOgenes$SYMBOL))),
                                      "CD8A", "CD8B", "CD3G", "CD3D", "CD3E", "CD19", "MS4A1", "CD79A", 
                                      "CD4", "IL7R", 
                                      "GZMK", "LYAR", "NKG7", "GZMA", "GNLY", "FGFBP2", "FCGR3A", "CCL4", "GZMH", 
                                      as.character(unlist(SGS.LS))))
